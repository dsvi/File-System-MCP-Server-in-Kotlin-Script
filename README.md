...
- **Tools provided:**
  ...
  | `write_file` | Write content to a file, or append to it via the optional `append` flag (default replace) |
  | `delete_file` | Delete a file, or a directory tree recursively |
  | `read_lines` | Read a line range `[from, to)` with 1-based, half-open indices (both optional, `from` defaults to 1) |
  | `change_lines` | Replace / insert / delete multiple 1-based, half-open line ranges in one atomic operation |
  | `run_shell` | Execute a shell command; returns `exit_code`, `stdout`, `stderr` separately, each clamped to `max_output` characters (default 1000) |
