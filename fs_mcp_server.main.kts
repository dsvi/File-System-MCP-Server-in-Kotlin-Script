...
fun readLines(filePath: String, from: Int, to: Int?): String {
    ...
    if (from < 1) throw ToolException("from ($from) must be at least 1.")
    ...
    if (from -1 > count) throw ToolException("from ($from) is beyond the file's line count ($count).")
    val end = to?.dec()?.coerceAtMost(count) ?: count
    val selected = lines.subList(from - 1, end).joinToString(separator = "")
    return selected
}
...
fun changeLines(filePath: String, changes: List<Change>) {
    ...
    for (i in changes.indices) {
        ...
        requireTool(fromLine >= 1) {
            "Invalid change $i: line indices must be at least 1 (from_line=$fromLine, to_line=$toLine)."
        }
    ...
    val sortedChanges = changes.sortedBy{ it.from }
    for (i in 0 until sortedChanges.size - 1) {
        ...
        if (nextFrom < currTo || nextFrom == currFrom) {
    ...
    for (i in changes.indices) {
        val fromLine = changes[i].from
        val toLine = changes[i].to
        requireTool(fromLine <= fileLineCount + 1) {
            "change $i: from_line ($fromLine) is beyond the file's line count ($fileLineCount)."
        }
        requireTool(toLine <= fileLineCount + 1) {
            "change $i: to_line ($toLine) is beyond the file's line count plus one ($fileLineCount + 1)."
        }
    }

    // Apply changes from bottom to top to preserve line numbers of earlier changes
    for ((fromLine1Based, toLine1Based, newContent) in changes.sortedByDescending { it.from }) {
        // Convert 1-based to 0-based indices for half-open range [from, to)
        val fromLine = fromLine1Based - 1
        val toLine = toLine1Based - 1
        
        // If inserting at the end (fromLine == lines.size), and the preceding line lacks a trailing newline, add one
        if (fromLine == lines.size && fromLine > 0){
        ...
        if (newContent.isNotEmpty()) {
            ...
            val last = newLines.lastIndex
            if (!newLines[last].endsWith("\n")) newLines[last] = newLines[last] + "\n"
        }

        // For half-open range [fromLine, toLine), clear and insert at fromLine
...
fun main() = runBlocking {
    ...
    server.addSafeTool(
        name = "read_lines",
        description = "Read the contents of a file by 1-based line numbers, using a half-open [from, to) range. Both from and to are optional; an omitted `to` means read till the end of the file.",
        inputSchema = ToolSchema(
            properties = buildJsonObject {
                prop("file_path", "string", "Path to the file to read.")
                prop("from", "integer", "1-based start line index (inclusive). Defaults to 1.")
                prop("to", "integer", "1-based end line index (exclusive). Omit to read till the end of the file.")
            },
            required = listOf("file_path")
        )
    ) { request ->
        CallToolResult(
            content = listOf(TextContent(readLines(request.required("file_path"), request.int("from") ?: 1, request.int("to"))))
        )
    }
    ...
    server.addSafeTool(
        ...
        description = """
            ...
            Each change is a tuple of (from_line, to_line, new_content) using 1-based line indexing
            and half-open ranges [from_line, to_line).

            - To replace lines: Provide from_line < to_line and new_content (e.g., [2, 4, "new"] replaces lines 2-3).
                ...
                putJsonObject("changes") {
                    ...
                    putJsonObject("items") {
                        ...
                        putJsonArray("prefixItems") {
                            prop("integer", "from_line: 1-based start line index (inclusive).")
                            prop("integer", "to_line: 1-based end line index (exclusive).")
